#!/usr/bin/env python
"""
meshforge - CLI Entry Point

Run the meshforge multi-agent system for autonomous MFEM-based FEA
dataset generation.

Usage:
    meshforge <mesh_file> [options]

Examples:
    meshforge inputs/model.mesh
    meshforge inputs/model.mesh --config configs/morphing_config.md
    meshforge inputs/model.mesh --generations 10 --population 5
"""
...
Examples:
  meshforge inputs/model.mesh
  meshforge inputs/model.mesh --config configs/morphing_config.md
  meshforge inputs/model.mesh --generations 10 --output outputs/run1
        """,
    )
...
async def main() -> int:
    ...
    # Import orchestrator
    try:
        from meshforge.orchestration.orchestrator import AgenticOrchestrator, OrchestrationConfig
        from meshforge.agents.llm.provider import create_provider, MockLLMProvider
