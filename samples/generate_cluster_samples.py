"""
Generate fine mesh samples with 3–5 blob holes (cluster) using scipy Delaunay.

No gmsh required.  Writes MFEM .mesh v1.0 directly.

All hole boundary edges receive tag 5 (traction-free by default).
Outer boundary tags: bottom=1, right=2, top=3, left=4.

Usage:
    python3 samples/generate_cluster_samples.py --n 300 --output train03
    python3 samples/generate_cluster_samples.py \\
        --n 300 --output train03 --near-boundary-fraction 0.3
"""
...
# ---------------------------------------------------------------------------
# Multi-hole interior point generation
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# Cluster hole placement
# ---------------------------------------------------------------------------

def _place_holes(n_holes, base_radius_range, near_boundary_fraction,
                 min_gap=0.04, max_attempts=300, seed=None):
    """
    Place n_holes non-overlapping blob holes in [0,1]^2.

    Returns list of ((cx, cy), radius).  May return fewer than n_holes
    if placement fails after max_attempts.
    """
    if seed is not None:
        np.random.seed(seed)
    ...
    for h in range(n_holes):
        ...
        # Near-boundary: smaller margin so hole can be close to edge
        margin_factor = 1.1 if near else 1.35
        margin_offset = 0.02 if near else 0.06

        for _ in range(max_attempts):
            r  = base_radius_range[0] + (
                 base_radius_range[1] - base_radius_range[0]) * np.random.random()
            mg = r * margin_factor + margin_offset
            # mg must be < 0.5 for the hole to fit at all
            if mg >= 0.49:
                mg = 0.48
...
def create_cluster_mesh(filename, n_holes_range=(3, 5),
                        near_boundary_fraction=0.3,
                        mesh_size_far=0.07, mesh_size_hole=0.025,
                        refinement_depth=0.08, seed=None):
    """Generate one plate with 3–5 non-overlapping blob holes."""
    if seed is not None:
        np.random.seed(seed)

    n_holes      = np.random.randint(n_holes_range[0], n_holes_range[1] + 1)
    base_r_range = (0.07, 0.13)   # smaller radii so multiple holes fit

    holes = _place_holes(n_holes, base_r_range, near_boundary_fraction, seed=seed)
    if len(holes) < 2:
        raise RuntimeError(
            f"Could only place {len(holes)} holes (needed ≥2). Try different seed."
        )

    hole_polys = [
        generate_blob_points(c, r, n_points=48, seed=(seed + h) if seed else None)
        for h, ((c), r) in enumerate(holes)
    ]

    outer_pts    = _make_boundary_pts(mesh_size_far)
    interior_pts = _make_interior_pts_multi(
        hole_polys, mesh_size_far, mesh_size_hole, refinement_depth
    )
    ...
    return len(triangles), len(holes)
...
def generate_all(output_dir, n_samples=300, mesh_size_far=0.07,
                 mesh_size_hole=0.025, refinement_depth=0.08, seed=42,
                 start_index=0, clear=True,
                 near_boundary_fraction=0.3,
                 n_holes_range=(3, 5)):
    np.random.seed(seed)
    ...
    print(f"Generating {n_samples} cluster meshes → {output_dir}/  (start={start_index})")
    print(f"  holes={n_holes_range[0]}–{n_holes_range[1]}  "
          f"near_boundary_fraction={near_boundary_fraction:.0%}  "
          f"mesh_size_far={mesh_size_far}  mesh_size_hole={mesh_size_hole}")
    ...
    for i in range(n_samples):
        ...
        try:
            n_e, n_h = create_cluster_mesh(
                filename,
                n_holes_range=n_holes_range,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Generate fine cluster-hole plate meshes (no gmsh required)"
    )
    ...
    parser.add_argument("--near-boundary-fraction",  type=float, default=0.3,
                        help="Fraction of holes placed near a boundary (default 0.3)")
    parser.add_argument("--min-holes",               type=int,   default=3)
    parser.add_argument("--max-holes",               type=int,   default=5)
    args = parser.parse_args()

    script_dir   = Path(__file__).parent
    project_root = script_dir.parent
    output_dir   = args.output or str(project_root / "train03")
    ...
    generate_all(
        ...
        n_holes_range          = (args.min_holes, args.max_holes),
    )
