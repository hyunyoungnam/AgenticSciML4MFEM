"""
demo.py — PIANO agentic SciML loop for edge-crack phase field fracture.

Loads pre-generated FEniCS samples from phase_field_data/, runs the full
agentic HPO loop (Critic → Architect → Physicist), and saves a 2×3
visualization panel.

Usage:
    python demo.py
    python demo.py --epochs 120 --rounds 6
    python demo.py --output my_demo.png

Prerequisites:
    python scripts/generate_phase_field_data.py   # generate training data first
"""
...
# ---------------------------------------------------------------------------
# Minimal mock LLM provider (no API key needed)
# ---------------------------------------------------------------------------
...
class MockLLMProvider:
    """Routes agent calls to hand-crafted responses; no network needed."""

    def __init__(self):
        self._arch_calls = 0

    async def generate(self, system_prompt: str, user_prompt: str, **kwargs) -> _MockResponse:
        sp = system_prompt.lower()
        if "training analyst" in sp:
            return _MockResponse(self._critic(user_prompt))
        if "reviewing a proposed configuration" in sp or "curve analyst" in sp:
            return _MockResponse("FEASIBLE: yes\nCONCERNS: none\nSUGGESTION: none\n"
                                 "OVERALL_PATTERN: underfitting\nSEVERITY: medium\n"
                                 "OBSERVATION: losses elevated.\nPINO_STATUS: inactive.\n"
                                 "ENSEMBLE_STATUS: ensemble_std=0.04")
        if "architect" in sp:
            return _MockResponse(self._architect(user_prompt))
        if "equilibrium" in sp:
            return _MockResponse(self._physicist(user_prompt))
        return _MockResponse("OVERALL_PATTERN: underfitting\nSEVERITY: medium\n"
                             "OBSERVATION: losses elevated.")

    def _critic(self, user_prompt: str) -> str:
        train_m = re.search(r'Final train loss:\s*([\d.eE+\-]+)', user_prompt)
        test_m  = re.search(r'Final test loss:\s*([\d.eE+\-]+)', user_prompt)
        if train_m and test_m:
            try:
                ratio = float(test_m.group(1)) / (float(train_m.group(1)) + 1e-12)
                if ratio > 5.0:
                    issue = "OVERFITTING"
                elif float(train_m.group(1)) < 0.01 and float(test_m.group(1)) < 0.05:
                    issue = "NONE"
                else:
                    issue = "UNDERFITTING"
            except ValueError:
            ...
            issue = "UNDERFITTING"
        return (
            f"DIAGNOSIS: {issue.lower()} detected.\n"
            f"PRIMARY_ISSUE: {issue}\nSEVERITY: {'low' if issue == 'NONE' else 'high'}\n"
            "RECOMMENDATIONS:\n- Increase d_model\n- Add energy physics loss\n"
            f"SHOULD_RETRAIN: {'false' if issue == 'NONE' else 'true'}\n"
            "METRICS_ANALYSIS:\n- gap: moderate\n"
        )

    def _architect(self, user_prompt: str) -> str:
        m = re.search(r'd_model:\s*(\d+)', user_prompt)
        d = min(int(m.group(1)) + 32, 256) if m else 96
        self._arch_calls += 1
        return (
            f"REASONING: Increase capacity to address underfitting.\n"
            f"CHANGES:\n- d_model: {d}\n- n_layers: 4\n- n_heads: 4\n"
            f"- slice_num: 16\n- learning_rate: 8e-4\n- energy: 0.05\n"
            "EXPECTED_IMPACT: Better fit across parameter space.\n"
            "TRADE_OFFS: Larger model, slightly slower.\n"
            "CONFIDENCE: medium\nCODE_CHANGE_DESCRIPTION: none\n"
        )

    def _physicist(self, user_prompt: str) -> str:
        def _pw(name):
            m = re.search(rf'- {name}:\s*([\d.e+\-]+)', user_prompt)
            return float(m.group(1)) if m else 0.0

        eq, en = _pw('equilibrium'), _pw('energy')
        if en == 0.0:
            changes = "- energy: 0.05"
            diag    = "No energy loss active — enabling elastic energy norm."
        elif eq == 0.0:
            changes = f"- energy: {en}\n- equilibrium: 0.001"
            diag    = "Energy stable. Enabling equilibrium residual."
        else:
            changes = ""
            diag    = "All active terms stable. Holding."
        return (
            f"PHYSICS_DIAGNOSIS: {diag}\nCHANGES:\n{changes}\n"
            "REASONING: Sequential enabling — each term stabilises before next.\n"
            "CONFIDENCE: high\n"
        )
...
def load_phase_field_dataset(
    ...
    if not params_list:
        raise RuntimeError(f"No valid samples found in {data_path}. "
                           "Run: python scripts/generate_phase_field_data.py")
...
def run_demo(
    ...
    print("=" * 70)
    print("PIANO: Agentic SciML Loop — Edge Crack Phase Field Demo")
    print("=" * 70)

    print(f"\n1. Loading phase field dataset ({n_samples} samples)...")
    params, all_coords, all_elements, all_outputs, all_damage = \
        load_phase_field_dataset(max_samples=n_samples)
    n_loaded = len(params)
    ...
    current_config = TransolverConfig(
        d_model=64, n_layers=3, n_heads=4, slice_num=16, dropout=0.05,
        learning_rate=1e-3, optimizer_type="adamw", scheduler_type="cosine",
        epochs=epochs_per_round, patience=epochs_per_round, batch_size=4,
        output_dim=3, equilibrium=0.0, energy=1e-4, tip_weight=0.0,
        near_tip=0.0, j_integral=0.0,
    )
    ...
    while round_idx < max_hpo_rounds and not converged:
        ...
        trainer = SurrogateTrainer(TrainingConfig(
            ...
            train_test_split=0.15, tip_coords=None,
        ...
        if result.success and curr_loss < best_test_loss:
            imp = (best_test_loss - curr_loss) / best_test_loss * 100 \
                  if best_test_loss < float('inf') else 100.0
        ...
        if not converged and round_idx < max_hpo_rounds - 1:
            ...
            for merged, _, _ in candidate_configs:
                ...
                bt = SurrogateTrainer(TrainingConfig(
                    ...
                    use_ensemble=False, train_test_split=0.15,
                    tip_coords=None, crack_config=crack_cfg,
                ))
    ...
    improvement      = (round_results[0]["test_loss"] - best_test_loss) \
                       / round_results[0]["test_loss"] * 100
